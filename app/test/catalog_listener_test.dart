...
void main() {
  group('CatalogServer.fromJson listeners', () {
    ...
    test(
      'synthesizes a single Reality listener when listeners is absent (old control-plane)',
      () {
        final server = CatalogServer.fromJson({
          'id': 1,
          'host': 'h',
    ...
    test(
      'synthesizes a single Reality listener when listeners is an empty array',
      () {
        final server = CatalogServer.fromJson({
          'id': 1,
          'host': 'h',
  ...
  group('CatalogServer.availableTransportParams', () {
    test(
      'maps reality to the empty-string param used by obfuscationModeQueryParam',
      () {
        final server = CatalogServer.fromJson({
          'id': 1,
          'host': 'h',
