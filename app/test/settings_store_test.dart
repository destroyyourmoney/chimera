...
void main() {
  ...
  group('ChimeraSettings', () {
    ...
    test(
      'fromJson migrates a settings file saved before SOCKS5 removal ("off") to dnsLeakGuard',
      () {
    ...
    test(
      'fromJson treats an unknown networkProtection string as dnsLeakGuard',
      () {
        final decoded = ChimeraSettings.fromJson({
          'networkProtection': 'garbage',
        });
        expect(decoded.networkProtection, NetworkProtectionMode.dnsLeakGuard);
      },
    );
    ...
    test(
      'subscriptionText assembles the #!chimera-subscription-v1 header and one link per line',
      () {
        final settings = ChimeraSettings(
          servers: [
    ...
    test(
      'subscriptionText with no servers is still a valid header-only document',
      () {
    ...
    test(
      'reordering servers changes subscriptionText line order (priority = order)',
      () {
        final settings = ChimeraSettings(
          servers: [
    ...
    test(
      'fromJson defaults splitTunnel to disabled/exclude/empty when absent',
      () {
        final decoded = ChimeraSettings.fromJson({});
  ...
  group('ObfuscationMode', () {
    ...
    test('fromJson defaults to reality when absent or unrecognized', () {
      expect(
        ChimeraSettings.fromJson({}).obfuscationMode,
        ObfuscationMode.reality,
      );
      expect(
        ChimeraSettings.fromJson({
          'obfuscationMode': 'garbage',
        }).obfuscationMode,
        ObfuscationMode.reality,
      );
    });
  ...
  group('obfuscationModeQueryParam', () {
    test(
      'maps all 4 modes to the query-param value subscription.go expects',
      () {
        expect(obfuscationModeQueryParam(ObfuscationMode.reality), '');
        expect(obfuscationModeQueryParam(ObfuscationMode.quicH3), 'quic');
        expect(
          obfuscationModeQueryParam(ObfuscationMode.shadowsocksAead),
          'ss',
        );
  ...
  group('applyObfuscationModeToCatalogServers', () {
      ...
      final settings = ChimeraSettings(
        servers: [
          ServerEntry(
            ...
            link:
                'chimera://185.100.157.232:443?pbk=abc&sni=example.com&fp=chrome#1',
          ),
        ],
      );

      settings.applyObfuscationModeToCatalogServers(
    ...
    test(
      'switching to Reality removes the mode= param entirely (matches _upsertCuratedServer)',
      () {
        final settings = ChimeraSettings(
          servers: [
            ServerEntry(
    ...
    test('leaves non-catalog (BYO/legacy) server links untouched', () {
      final settings = ChimeraSettings(
        servers: [
          ServerEntry(
            id: 'byo-1',
            label: 'My server',
            link: 'chimera://host:443?pbk=x&mode=quic',
          ),
        ],
      );
      ...
      expect(
        settings.servers.single.link,
        'chimera://host:443?pbk=x&mode=quic',
      );
    });
      ...
      final settings = ChimeraSettings(
        servers: [
          ServerEntry(
            id: 'catalog-1',
            label: 'a',
            link: 'chimera://a:443?pbk=x',
          ),
          ServerEntry(
            id: 'catalog-2',
            label: 'b',
            link: 'chimera://b:443?pbk=y',
          ),
        ],
      );
  ...
  group(
    'applyObfuscationModeToCatalogServers with catalogListeners (per-transport ports)',
    () {
      test(
        'rewrites both mode= and the port to the target transport\'s own listener port',
        () {
          final settings = ChimeraSettings(
            servers: [
              ServerEntry(
      ...
      test(
        'switching back to Reality restores the reality listener\'s port and drops mode=',
        () {
          final settings = ChimeraSettings(
            servers: [
              ServerEntry(
      ...
      test(
        'leaves a server unchanged entirely when it has no listener for the target transport',
        () {
      ...
      test(
        'falls back to mode-only rewrite (legacy behavior) when catalogListeners is empty',
        () {
          final settings = ChimeraSettings(
            servers: [
              ServerEntry(
  ...
  group('favoriteServerIds', () {
    test('round-trips through JSON', () {
      final settings = ChimeraSettings(
        favoriteServerIds: ['se-sto-1', 'nl-ams-1'],
      );
      final decoded = ChimeraSettings.fromJson(settings.toJson());
