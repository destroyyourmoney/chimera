...
// ServerHelloShape pins the wire-observable shape of a TLS 1.3 server's own
// ServerHello (cipher suite, negotiated group, and the order of the
// supported_versions / key_share extensions) to match a template captured
// from a real peer, instead of this stack's own fixed negotiation order and
// serialization order. It only affects a fresh (non-HelloRetryRequest,
// non-resumption) ServerHello -- the only kind CHIMERA's Reality server
// sends.
//
// If ForceCipherSuite/ForceGroup name a suite/group the client did not
// actually offer, they are silently ignored (negotiation falls back to this
// stack's normal preference order) rather than failing the handshake --
// forcing a HelloRetryRequest round trip here would itself be a timing/shape
// oracle. [CHIMERA]
type ServerHelloShape struct {
	// ForceCipherSuite, if non-zero and present in the ClientHello's
	// cipher_suites, is negotiated instead of this stack's preference order.
	ForceCipherSuite uint16

	// ForceGroup, if non-zero and the client sent a key_share for it, is
	// negotiated instead of this stack's preference order. Never triggers a
	// HelloRetryRequest.
	ForceGroup CurveID

	// ExtensionOrder, if it is a permutation of
	// {extensionSupportedVersions (43), extensionKeyShare (51)}, controls
	// the emitted order of those two extensions. Any other value (nil,
	// wrong length, wrong contents) is ignored and the default
	// supported_versions-then-key_share order is used.
...
type Config struct {
	...
	// ServerHelloShape, if non-nil, lets a TLS 1.3 server handshake (server
	// side only -- no effect on client handshakes) force the
	// wire-observable shape of its own ServerHello to match one captured
	// from a real peer, instead of this stack's own negotiation/
	// serialization order. See ServerHelloShape's docs. [CHIMERA]
	ServerHelloShape *ServerHelloShape // [CHIMERA]
...
func (c *Config) Clone() *Config {
	...
	return &Config{
		...
		ServerHelloShape:                   c.ServerHelloShape,                  // [CHIMERA]
	}
}
