...
type serverHelloMsg struct {
	...
	// [CHIMERA] extensionOrder, if it is the permutation
	// {extensionSupportedVersions, extensionKeyShare} (in either order),
	// controls the emitted order of those two extensions instead of the
	// hardcoded supported_versions-then-key_share default. Any other value
	// (nil, wrong length, wrong contents) is ignored. See ServerHelloShape.
...
func (m *serverHelloMsg) marshal() ([]byte, error) {
	...
	// [CHIMERA] These two extensions are the only ones a fresh (non-HRR,
	// non-resumption) TLS 1.3 ServerHello ever carries in this codebase, and
	// their relative order is part of the JA3S fingerprint. Emit them in
	// extensionOrder when the caller (ServerHelloShape) pinned one, so the
	// server can reproduce a real peer's observed order; otherwise fall back
	// to the original hardcoded supported_versions-then-key_share order.
	writeSupportedVersions := func() {
	...
	if len(m.extensionOrder) == 2 &&
		m.extensionOrder[0] != m.extensionOrder[1] &&
		(m.extensionOrder[0] == extensionSupportedVersions || m.extensionOrder[0] == extensionKeyShare) &&
		(m.extensionOrder[1] == extensionSupportedVersions || m.extensionOrder[1] == extensionKeyShare) {
		for _, t := range m.extensionOrder {
			if t == extensionSupportedVersions {
