...
func (hs *serverHandshakeStateTLS13) processClientHello() error {
	...
	// [CHIMERA] Pin the supported_versions/key_share extension order to a
	// captured template, if one was configured. Validated at marshal time;
	// an invalid value here is simply ignored there.
	if shape := c.config.ServerHelloShape; shape != nil {
		hs.hello.extensionOrder = shape.ExtensionOrder
	}
	...
	// [CHIMERA] A pinned ServerHelloShape overrides this stack's own cipher
	// preference order, so the negotiated suite can match a real peer's
	// choice. Only takes effect if the client actually offered it -- never
	// fails the handshake, just falls through to normal negotiation.
	if shape := c.config.ServerHelloShape; shape != nil && shape.ForceCipherSuite != 0 {
		hs.suite = mutualCipherSuiteTLS13(hs.clientHello.cipherSuites, shape.ForceCipherSuite)
	}
	...
	// [CHIMERA] As with the cipher suite above: prefer a pinned group over
	// this stack's own preference order, but only when the client already
	// sent a key_share for it -- forcing a group the client didn't offer
	// would trigger a HelloRetryRequest round trip, itself a shape leak.
	if shape := c.config.ServerHelloShape; shape != nil && shape.ForceGroup != 0 &&
		slices.Contains(preferredGroups, shape.ForceGroup) && hasKeyShare(shape.ForceGroup) {
		selectedGroup = shape.ForceGroup
	}
