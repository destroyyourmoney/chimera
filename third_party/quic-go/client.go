...
// chromeH3InitialDestConnIDLen is the fixed destination connection ID length
// real Chrome uses for its first Initial packet's DCID guess (pcap-derived
// reference, see third_party/quic-go/internal/wire/transport_parameters.go
// marshalClientChromeH3). Stock quic-go instead randomizes this length
// between 8 and 20 bytes (protocol.GenerateConnectionIDForInitial), which
// would itself be a distinguishing signal for the chrome-h3 profile.
const chromeH3InitialDestConnIDLen = 8

// generateConnectionIDForInitialWithProfile picks the client's initial DCID
// guess. For the chrome-h3 fingerprint profile it uses Chrome's fixed
// 8-byte length; every other profile (including the default empty one) keeps
// quic-go's stock randomized-length behavior unchanged.
func generateConnectionIDForInitialWithProfile(profile string) (protocol.ConnectionID, error) {
	if profile == "chrome-h3" {
		return protocol.GenerateConnectionID(chromeH3InitialDestConnIDLen)
	}
