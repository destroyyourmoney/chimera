...
func populateConfig(config *Config) *Config {
	...
	return &Config{
		...
		UseElasticCC:                     config.UseElasticCC,       // ElasticCC patch: must survive config cloning
		ElasticCCBandwidth:               config.ElasticCCBandwidth, // Brutal fixed-rate target (bytes/s); 0 = adaptive
		InitialCryptoDataTracer:          config.InitialCryptoDataTracer,
		InitialPacketTracer:              config.InitialPacketTracer,
		InitialClientHelloProfile:        config.InitialClientHelloProfile,
	}
}
