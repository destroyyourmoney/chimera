...
type packetPacker struct {
	...
	// chromeH3Initial gates the Chrome-shaped Initial-CRYPTO-fragmentation
	// path in maybeGetCryptoPacket (see packChromeH3InitialCryptoFrames):
	// only the chrome-h3 fingerprint profile's client Initial packets use
	// it. It is never true for the server side.
	chromeH3Initial bool
...
func newPacketPacker(
	...
	return &packetPacker{
		...
		pnManager:           packetNumberManager,
		chromeH3Initial:     chromeH3Initial,
	}
}
...
func (p *packetPacker) maybeGetCryptoPacket(
	...
	if hasRetransmission {
		...
		return hdr, pl
	} else if p.chromeH3Initial && encLevel == protocol.EncryptionInitial {
		p.packChromeH3InitialCryptoFrames(&pl, popCryptoFrame, hasCryptoData, handler, maxPacketSize, v)
	} else {
		for hasCryptoData() {
...
const (
	chromeH3MinCryptoFragments = 3
	chromeH3MaxCryptoFragments = 10
	chromeH3MinPINGFrames      = 1
	chromeH3MaxPINGFrames      = 10
)

// packChromeH3InitialCryptoFrames fragments the client Initial CRYPTO stream
// into several smaller frames interspersed with PING frames, mirroring real
// Chrome's observed Initial packet layout: a live capture of Chrome 150
// stable (see docs/uquic-initial-fingerprint.md, increment 10) showed CRYPTO
// fragments as small as 1 byte and as large as 618 bytes, interleaved with
// PING frames — not one or two large CRYPTO frames followed by PADDING,
// which is what this code emitted before and is itself a distinguishing
// signal (real Chrome never does that).
//
// The exact fragment sizes/positions are randomized per packet rather than
// an attempt to reproduce Chrome's specific internal buffering byte-for-byte
// (which looks like an artifact of Chrome's own internal TLS/socket-write
// timing, not a stable, specified protocol behavior) — the goal is matching
// the observable *shape* (many small CRYPTO frames + PING, varying per
// connection), the same anti-fixed-pattern rationale as the transport
// parameter order shuffle in internal/wire's marshalClientChromeH3.
func (p *packetPacker) packChromeH3InitialCryptoFrames(
	pl *payload,
	popCryptoFrame func(protocol.ByteCount) *wire.CryptoFrame,
	hasCryptoData func() bool,
	handler ackhandler.FrameHandler,
	maxPacketSize protocol.ByteCount,
	v protocol.Version,
) {
	numCryptoFrames := chromeH3MinCryptoFragments + int(p.rand.Int32N(int32(chromeH3MaxCryptoFragments-chromeH3MinCryptoFragments+1)))
	numPING := chromeH3MinPINGFrames + int(p.rand.Int32N(int32(chromeH3MaxPINGFrames-chromeH3MinPINGFrames+1)))

	// Build a shuffled sequence of "crypto" and "ping" slots. Crypto slots
	// still pop data strictly in stream order (required by the protocol);
	// only their sizes and interleaving with PING frames are randomized.
	slots := make([]bool, 0, numCryptoFrames+numPING) // true = crypto slot
	for range numCryptoFrames {
		slots = append(slots, true)
	}
	for range numPING {
		slots = append(slots, false)
	}
	p.rand.Shuffle(len(slots), func(i, j int) { slots[i], slots[j] = slots[j], slots[i] })

	remainingCryptoSlots := numCryptoFrames
	for _, isCrypto := range slots {
		if maxPacketSize <= 0 {
			break
		}
		if !isCrypto {
		...
		if !hasCryptoData() {
			remainingCryptoSlots--
			continue
		}
		// Divide the remaining budget across the remaining crypto slots so
		// early fragments don't starve later ones; the observed real
		// captures mix small and large fragments rather than strictly equal
		// ones, which this simple even split approximates well enough given
		// the goal is shape, not an exact byte-for-byte match.
		chunkCap := maxPacketSize
		if remainingCryptoSlots > 1 {
			chunkCap = maxPacketSize / protocol.ByteCount(remainingCryptoSlots)
			if chunkCap < 1 {
				chunkCap = 1
			}
		}
		cf := popCryptoFrame(chunkCap)
		remainingCryptoSlots--
		if cf == nil {
			continue
		}
	...
	// Drain any crypto data that didn't fit within the fragment budget above
	// (e.g. more data queued than slots could carry) so nothing is silently
	// left unsent; this uses the stock greedy behavior for the remainder.
	for hasCryptoData() {
		cf := popCryptoFrame(maxPacketSize)
		if cf == nil {
			break
...
func (p *packetPacker) appendLongHeaderPacket(buffer *packetBuffer, header *wire.ExtendedHeader, pl payload, padding protocol.ByteCount, encLevel protocol.EncryptionLevel, sealer sealer, v protocol.Version) (*longHeaderPacket, error) {
	...
	chromeInitialLayout := p.perspective == protocol.PerspectiveClient && encLevel == protocol.EncryptionInitial
	raw, err = p.appendPacketPayload(raw, pl, paddingLen, v, chromeInitialLayout, chromeInitialLayout)
	if err != nil {
		return nil, err
	}
...
func (p *packetPacker) appendShortHeaderPacket(
	...
	raw, err = p.appendPacketPayload(raw, pl, paddingLen, v, false, false)
	if err != nil {
		return shortHeaderPacket{}, err
	}
	if !isMTUProbePacket {
...
// It modifies the order of payload.frames unless stableFrames is set.
func (p *packetPacker) appendPacketPayload(raw []byte, pl payload, paddingLen protocol.ByteCount, v protocol.Version, paddingAfterFrames, stableFrames bool) ([]byte, error) {
	...
	if paddingLen > 0 && !paddingAfterFrames {
	...
	if len(pl.frames) > 1 && !stableFrames {
