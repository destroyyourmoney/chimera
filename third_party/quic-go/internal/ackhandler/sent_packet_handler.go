...
func NewSentPacketHandler(
	...
	if useElasticCC {
		cc = congestion.NewElasticSender(congestion.DefaultClock{}, rttStats, initialMaxDatagramSize, elasticBandwidth)
	} else {
...
// elasticMaxPTOShift caps the PTO exponential backoff under ElasticCC. In an
// adversarial-loss environment, loss is NOT congestion, so the probe timer must
// not back off toward minutes (default behavior climbs to maxPTODuration = 60s)
// — that idle-gap backoff, not the congestion window, is what stalls a bulk
// transfer under heavy loss. Capping the shift keeps retransmission probes
// frequent (≤ ~2× base PTO) so lost data is recovered promptly.
const elasticMaxPTOShift = 1

func (h *sentPacketHandler) getScaledPTO(includeMaxAckDelay bool) time.Duration {
	shift := h.ptoCount
	if h.useElasticCC && shift > elasticMaxPTOShift {
		shift = elasticMaxPTOShift
	}
	pto := h.rttStats.PTO(includeMaxAckDelay) << shift
...
func (h *sentPacketHandler) MigratedPath(now monotime.Time, initialMaxDatagramSize protocol.ByteCount) {
	...
	if h.useElasticCC {
		h.congestion = congestion.NewElasticSender(congestion.DefaultClock{}, h.rttStats, initialMaxDatagramSize, h.elasticBandwidth)
	} else {
