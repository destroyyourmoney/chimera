...
type cryptoSetup struct {
	...
	initialCryptoDataTracer func([]byte)
...
func NewCryptoSetupClient(
	...
	version protocol.Version,
	initialCryptoDataTracer func([]byte),
	initialClientHelloProfile string,
) CryptoSetup {
	cs := newCryptoSetup(
		connID,
		tp,
	...
	cs.initialCryptoDataTracer = initialCryptoDataTracer
	...
	marshaledParams := cs.ourParams.MarshalForProfile(protocol.PerspectiveClient, initialClientHelloProfile)
	cs.conn = newClientQUICConn(tlsConf, marshaledParams, initialClientHelloProfile)
	cs.conn.SetTransportParameters(marshaledParams)

	return cs
}

func newClientQUICConn(tlsConf *tls.Config, transportParameters []byte, initialClientHelloProfile string) quicTLSConn {
	if initialClientHelloProfile == "chrome-h3" {
		if conn, err := newChromeH3QUICClient(tlsConf, transportParameters); err == nil {
			return conn
		}
	}
	return stdlibQUICConn{QUICConn: tls.QUICClient(&tls.QUICConfig{
		TLSConfig:           tlsConf,
		EnableSessionEvents: true,
	})}
}

func newChromeH3QUICClient(tlsConf *tls.Config, transportParameters []byte) (quicTLSConn, error) {
	...
	setChromeH3QUICExtensions(&spec, transportParameters)
	conn := utls.UQUICClient(&utls.QUICConfig{
		TLSConfig: utlsConfigFromTLS(tlsConf),
	}, utls.HelloCustom)
	if err := conn.ApplyPreset(&spec); err != nil {
		return nil, err
	}
	conn.SetTransportParameters(transportParameters)
	return utlsQUICConn{conn: conn}, nil
}

func setChromeH3QUICExtensions(spec *utls.ClientHelloSpec, transportParameters []byte) {
	hasQUICParams := false
	for i, ext := range spec.Extensions {
		switch ext.(type) {
		case *utls.ALPNExtension:
			spec.Extensions[i] = &utls.ALPNExtension{AlpnProtocols: []string{"h3"}}
		...
		case *utls.QUICTransportParametersExtension:
			hasQUICParams = true
			spec.Extensions[i] = &utls.QUICTransportParametersExtension{
				TransportParameters: utlsTransportParametersFromWire(transportParameters),
			}
		}
	}
	if !hasQUICParams {
...
func utlsTransportParametersFromWire(b []byte) utls.TransportParameters {
	params := make(utls.TransportParameters, 0, 16)
	for len(b) > 0 {
		...
		b = b[n:]
		val := append([]byte(nil), b[:l]...)
		b = b[l:]
		params = append(params, &utls.FakeQUICTransportParameter{Id: id, Val: val})
	}
	return params
}

func utlsConfigFromTLS(c *tls.Config) *utls.Config {
	return &utls.Config{
		Rand:               c.Rand,
		Time:               c.Time,
		RootCAs:            c.RootCAs,
...
func (u utlsQUICConn) Start(ctx context.Context) error { return u.conn.Start(ctx) }

func (u utlsQUICConn) NextEvent() tls.QUICEvent {
	ev := u.conn.NextEvent()
	data := ev.Data
	if ev.Kind == utls.QUICWriteData {
		data = unwrapTLSHandshakeRecords(data)
	}
...
func unwrapTLSHandshakeRecords(data []byte) []byte {
	if len(data) < 5 || data[0] != 0x16 {
		return data
	}
	var out []byte
	for len(data) >= 5 && data[0] == 0x16 {
		l := int(data[3])<<8 | int(data[4])
		if len(data) < 5+l {
			return data
		}
		out = append(out, data[5:5+l]...)
		data = data[5+l:]
	}
	if len(data) != 0 {
		return data
...
func (u utlsQUICConn) Close() error { return u.conn.Close() }

func (u utlsQUICConn) HandleData(level tls.QUICEncryptionLevel, data []byte) error {
	return u.conn.HandleData(utls.QUICEncryptionLevel(level), data)
}

func (u utlsQUICConn) SetTransportParameters(params []byte) {
	u.conn.SetTransportParameters(params)
}

func (u utlsQUICConn) SendSessionTicket(tls.QUICSessionTicketOptions) error {
	return errors.New("utls QUIC client does not send session tickets")
}

func (u utlsQUICConn) StoreSession(*tls.SessionState) error {
	return errors.New("utls QUIC client uses its own session cache")
}

func (u utlsQUICConn) ConnectionState() tls.ConnectionState {
	s := u.conn.ConnectionState()
...
func (h *cryptoSetup) writeRecord(encLevel tls.QUICEncryptionLevel, p []byte) {
	...
	case tls.QUICEncryptionLevelInitial:
		if h.initialCryptoDataTracer != nil && h.perspective == protocol.PerspectiveClient {
			h.initialCryptoDataTracer(append([]byte(nil), p...))
		}
