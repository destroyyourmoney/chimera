...
func TestInitialCryptoDataTracerReceivesClientHelloCopy(t *testing.T) {
	...
	cl := NewCryptoSetupClient(
		...
		utils.DefaultLogger.WithPrefix("client"),
		protocol.Version1,
		func(p []byte) { traced = p },
		"",
	)

	require.NoError(t, cl.StartHandshake(context.Background()))
	require.NotEmpty(t, traced)
	require.Equal(t, byte(typeClientHello), traced[0])

	traced[0] = 0xff
	ev := cl.NextEvent()
	require.Equal(t, EventWriteInitialData, ev.Kind)
	require.NotEmpty(t, ev.Data)
	require.Equal(t, byte(typeClientHello), ev.Data[0])
}
