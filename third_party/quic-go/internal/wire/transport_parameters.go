...
// Marshal the transport parameters using quic-go's stock parameter order.
func (p *TransportParameters) Marshal(pers protocol.Perspective) []byte {
	return p.MarshalForProfile(pers, "")
}

// MarshalForProfile marshals the transport parameters. When pers is
// PerspectiveClient and profile is "chrome-h3", the client parameters are
// emitted in Chrome's observable order (see marshalClientChromeH3); any other
// profile value falls back to the stock order so that non-fingerprinted
// clients never emit a Chrome-shaped parameter order without also matching
// Chrome's ClientHello (which would itself be a distinguishing oracle).
func (p *TransportParameters) MarshalForProfile(pers protocol.Perspective, profile string) []byte {
	...
	if pers == protocol.PerspectiveClient && profile == "chrome-h3" {
		return p.marshalClientChromeH3(b)
	}
...
// googleConnectionOptionsParameterID and googleInitialRTTParameterID are
// Chrome/QUICHE-proprietary (non-IETF) transport parameters observed on the
// wire in real Chrome QUIC Initial packets. They carry no negotiated meaning
// for quic-go's own connection and exist here purely so the chrome-h3
// fingerprint's parameter *set* matches Chrome's, not just individual values.
const (
...
// chromeH3WireMaxIdleTimeoutMs is the max_idle_timeout value (milliseconds)
// real Chrome presents on the wire (300000ms / 5 minutes), verified against a
// live capture of Chrome 150 stable. This is a wire-presentation-only value:
// it does NOT change CHIMERA's own actual idle-timeout enforcement
// (p.MaxIdleTimeout, set by internal/quic's carrier config), the same way
// google_connection_options/google_initial_rtt below carry no operational
// meaning for this connection — max_idle_timeout is advisory/negotiated
// (RFC 9000 §10.1: each side's effective timeout is the min of its own
// configured value and the peer's advertised one), so presenting a larger
// advertised value than we actually enforce is protocol-safe.
const chromeH3WireMaxIdleTimeoutMs = 300000

// marshalClientChromeH3 emits the client transport parameters the way real
// Chrome's QUIC stack does, based on a pcap-derived reference (Chrome
// 115/146): a specific parameter *set* (several IETF parameters Chrome never
// sends are omitted; two Google-proprietary parameters are added) written in
// a freshly randomized order on every call. Chrome reshuffles the parameter
// order on every handshake — a fixed order would itself be a distinguishing
// signal, so this must not be made deterministic.
//
// initial_source_connection_id is passed through rather than spoofed, since
// it must stay consistent with the connection ID actually used on the wire
// (the peer validates it against the packet header) — that consistency is
// handled separately, at the connection-ID generation layer (see
// generateConnectionIDForInitialWithProfile in client.go), not here.
//
// Values verified against a live capture of real Chrome 150 stable
// (see docs/uquic-initial-fingerprint.md): initial_max_data,
// initial_max_stream_data_{bidi_local,bidi_remote,uni},
// initial_max_streams_{bidi,uni}, max_udp_payload_size,
// max_datagram_frame_size, and the version_information GREASE-version shape
// all matched exactly. max_idle_timeout did not (real Chrome sends 300000ms,
// not 30000ms) and is now pinned via chromeH3WireMaxIdleTimeoutMs.
// google_connection_options' value could not be verified this way: the local
// capture required Chrome's --origin-to-force-quic-on debug flag (no public
// server + Alt-Svc infrastructure was available to trigger QUIC normally),
// which appears to alter this specific value; the pinned "10AF" here is
// from the earlier uquic-derived reference, not the live capture.
func (p *TransportParameters) marshalClientChromeH3(b []byte) []byte {
	var entries [][]byte

	appendVarintParam := func(id transportParameterID, val uint64) {
		e := quicvarint.Append(nil, uint64(id))
		e = quicvarint.Append(e, uint64(quicvarint.Len(val)))
		e = quicvarint.Append(e, val)
		entries = append(entries, e)
	}
	appendRawParam := func(id transportParameterID, val []byte) {
		e := quicvarint.Append(nil, uint64(id))
		e = quicvarint.Append(e, uint64(len(val)))
		e = append(e, val...)
		entries = append(entries, e)
	}

	appendVarintParam(initialMaxStreamDataBidiLocalParameterID, uint64(p.InitialMaxStreamDataBidiLocal))
	appendVarintParam(initialMaxStreamDataBidiRemoteParameterID, uint64(p.InitialMaxStreamDataBidiRemote))
	appendVarintParam(initialMaxStreamDataUniParameterID, uint64(p.InitialMaxStreamDataUni))
	appendVarintParam(initialMaxDataParameterID, uint64(p.InitialMaxData))
	appendVarintParam(initialMaxStreamsBidiParameterID, uint64(p.MaxBidiStreamNum))
	appendVarintParam(initialMaxStreamsUniParameterID, uint64(p.MaxUniStreamNum))
	appendVarintParam(maxIdleTimeoutParameterID, chromeH3WireMaxIdleTimeoutMs)
	if p.MaxUDPPayloadSize > 0 {
		appendVarintParam(maxUDPPayloadSizeParameterID, uint64(p.MaxUDPPayloadSize))
	}
	if p.MaxDatagramFrameSize != protocol.InvalidByteCount {
		appendVarintParam(maxDatagramFrameSizeParameterID, uint64(p.MaxDatagramFrameSize))
	}
	appendRawParam(initialSourceConnectionIDParameterID, p.InitialSourceConnectionID.Bytes())

	// version_information (RFC 9368): chosen version + a GREASE version to
	// resist version ossification, mirroring Chrome's own anti-ossification
	// GREASE usage.
	versionInfo := make([]byte, 0, 8)
	versionInfo = binary.BigEndian.AppendUint32(versionInfo, quicVersion1)
	versionInfo = binary.BigEndian.AppendUint32(versionInfo, greaseVersion())
	appendRawParam(versionInformationParameterID, versionInfo)

	// google_connection_options: fixed 4-byte tag, stable across a given
	// Chrome release (observed value for Chrome 146).
	appendRawParam(googleConnectionOptionsParameterID, []byte("10AF"))

	// google_initial_rtt: Chrome sends a randomized initial RTT estimate
	// (microseconds) in the range [1000, 20000] on every connection.
	appendVarintParam(googleInitialRTTParameterID, googleInitialRTT())

	// GREASE, shuffled in with the rest rather than always leading — real
	// Chrome's GREASE transport parameter is not pinned to a fixed position.
	appendRawParam(greaseTransportParameterID(), greaseTransportParameterValue())

	shuffleByteSlices(entries)
...
// greaseVersion returns a reserved QUIC version of the form 0x?a?a?a?a, as
// recommended by RFC 9368 for anti-ossification GREASE.
func greaseVersion() uint32 {
	var r [4]byte
	rand.Read(r[:])
	var v uint32
	for i, byteVal := range r {
		nibble := byteVal & 0xf0
		v |= (uint32(nibble) | 0xa) << (8 * uint(3-i))
	}
	return v
}

// greaseTransportParameterID returns a GREASE transport-parameter ID of the
// form 31*N+27, per RFC 9000 §18.1.
func greaseTransportParameterID() transportParameterID {
	var r [1]byte
	rand.Read(r[:])
	return transportParameterID(27 + 31*uint64(r[0]))
}

func greaseTransportParameterValue() []byte {
	var length [1]byte
	rand.Read(length[:])
	val := make([]byte, length[0]%16)
	rand.Read(val)
	return val
}

// googleInitialRTT returns a randomized initial RTT estimate in microseconds,
// matching the range Chrome sends in its google_initial_rtt parameter.
func googleInitialRTT() uint64 {
	var r [4]byte
	rand.Read(r[:])
	const span = uint32(20000 - 1000)
	return 1000 + uint64(binary.BigEndian.Uint32(r[:])%span)
}

// shuffleByteSlices randomizes the order of already-encoded parameter
// entries in place using a Fisher-Yates shuffle.
func shuffleByteSlices(entries [][]byte) {
	for i := len(entries) - 1; i > 0; i-- {
		var r [4]byte
		rand.Read(r[:])
		j := int(binary.BigEndian.Uint32(r[:]) % uint32(i+1))
