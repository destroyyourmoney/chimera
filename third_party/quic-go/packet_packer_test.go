...
func TestClientInitialPayloadCryptoBeforePadding(t *testing.T) {
	...
	tp.pnManager.EXPECT().PeekPacketNumber(protocol.EncryptionInitial).Return(protocol.PacketNumber(0x24), protocol.PacketNumberLen2)
	tp.pnManager.EXPECT().PopPacketNumber(protocol.EncryptionInitial).Return(protocol.PacketNumber(0x24))
	tp.sealingManager.EXPECT().GetInitialSealer().Return(sealer, nil)
	...
	hdr, _, _, err := wire.ParsePacket(p.buffer.Data)
	require.NoError(t, err)
	extHdr, err := hdr.ParseExtended(p.buffer.Data)
	require.NoError(t, err)
	payload := p.buffer.Data[extHdr.ParsedLen():]
	payload = payload[:len(payload)-sealer.Overhead()]

	parser := wire.NewFrameParser(false, false, false)
	var numCryptoFrames int
	for pos := 0; pos < len(payload); {
		if payload[pos] == 0 {
			padding := payload[pos:]
			require.NotEmpty(t, padding)
			require.Equal(t, make([]byte, len(padding)), padding)
			require.Greater(t, numCryptoFrames, 0)
			return
		}
		frameType, frameTypeLen, err := parser.ParseType(payload[pos:], protocol.EncryptionInitial)
		require.NoError(t, err)
		require.Equal(t, wire.FrameTypeCrypto, frameType)
		require.Equal(t, 1, frameTypeLen, "ParseType must not skip PADDING before CRYPTO")
		frame, frameLen, err := parser.ParseLessCommonFrame(frameType, payload[pos+frameTypeLen:], protocol.Version1)
		require.NoError(t, err)
		require.IsType(t, &wire.CryptoFrame{}, frame)
		pos += frameTypeLen + frameLen
		numCryptoFrames++
	}
	t.Fatal("missing trailing PADDING")
}
