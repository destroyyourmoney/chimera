...
type Config struct {
	...
	// ElasticCCBandwidth, when > 0, puts ElasticCC into fixed-rate "Brutal" mode:
	// instead of estimating bandwidth (which is suppressed under heavy loss because
	// ACKs are themselves lost), it paces at this target rate (bytes/s) divided by
	// the loss-success ratio. Set to the link's known capacity to hold goodput at
	// 30–40 % loss. 0 = adaptive estimation (default). Only used with UseElasticCC.
	ElasticCCBandwidth uint64

	// InitialCryptoDataTracer, if set on a client Config, is called with a copy of
	// each TLS CRYPTO chunk emitted at Initial encryption level. This Chimera patch
	// is observability-only: it must not mutate the handshake or packet contents.
	InitialCryptoDataTracer func([]byte)

	// InitialPacketTracer, if set on a client Config, is called with a copy of
	// each sent Initial packet and stable metadata about the packet surface.
	// This Chimera patch is observability-only for fingerprint validation.
	InitialPacketTracer func(InitialPacketTrace)

	// InitialClientHelloProfile selects an alternate client-side QUIC Initial
	// ClientHello generator. Empty keeps the stock crypto/tls path.
	InitialClientHelloProfile string

	Tracer func(ctx context.Context, isClient bool, connID ConnectionID) qlogwriter.Trace
}

// InitialPacketTrace describes a sent client Initial packet without exposing
// quic-go internal types. Packet contains the encrypted wire bytes for this
// Initial packet only, not any following coalesced packets.
type InitialPacketTrace struct {
	Packet           []byte
	Version          Version
