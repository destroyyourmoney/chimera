...
var newClientConnection = func(
	...
	cs := handshake.NewCryptoSetupClient(
		...
		logger,
		s.version,
		s.config.InitialCryptoDataTracer,
		s.config.InitialClientHelloProfile,
	)
	s.cryptoStreamHandler = cs
	s.cryptoStreamManager = newCryptoStreamManager(s.initialStream, s.handshakeStream, oneRTTStream)
	s.unpacker = newPacketUnpacker(cs, s.srcConnIDLen)
	s.packer = newPacketPacker(srcConnID, s.connIDManager.Get, s.initialStream, s.handshakeStream, s.sentPacketHandler, s.retransmissionQueue, cs, s.framer, &s.receivedPacketHandler, s.datagramQueue, s.perspective, s.config.InitialClientHelloProfile == "chrome-h3")
...
func (c *Conn) traceInitialPacket(packet *coalescedPacket, p *longHeaderPacket) {
	if c.config.InitialPacketTracer == nil ||
		c.perspective != protocol.PerspectiveClient ||
		p.EncryptionLevel() != protocol.EncryptionInitial {
		return
	}
	packetLen := int(p.length)
	if packetLen > len(packet.buffer.Data) {
		packetLen = len(packet.buffer.Data)
	}
	frames := make([]string, 0, len(p.frames)+2)
	if p.ack != nil {
		frames = append(frames, "ack")
	}
	for _, f := range p.frames {
		frames = append(frames, initialPacketFrameName(f.Frame))
	}
	for range p.streamFrames {
		frames = append(frames, "stream")
	}
	if packetLen >= int(c.config.InitialPacketSize) {
		frames = append(frames, "padding")
	}
	c.config.InitialPacketTracer(InitialPacketTrace{
		Packet:           append([]byte(nil), packet.buffer.Data[:packetLen]...),
		Version:          p.header.Version,
		DestConnectionID: append([]byte(nil), p.header.DestConnectionID.Bytes()...),
		SrcConnectionID:  append([]byte(nil), p.header.SrcConnectionID.Bytes()...),
		Token:            append([]byte(nil), p.header.Token...),
		PacketNumber:     uint64(p.header.PacketNumber),
		PacketNumberLen:  int(p.header.PacketNumberLen),
		Length:           packetLen,
		Frames:           frames,
	})
}

func initialPacketFrameName(f wire.Frame) string {
	switch f.(type) {
	case *wire.CryptoFrame:
		return "crypto"
	case *wire.PingFrame:
